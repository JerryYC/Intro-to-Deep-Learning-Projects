...
def get_datasets(config_data):
    ...
    train_data_loader = get_coco_dataloader(train_ids_file_path, root_train, 
      train_annotation_file, coco, vocabulary, config_data)
    ...
    test_data_loader = get_coco_dataloader(test_ids_file_path, root_test, 
      test_annotation_file, coco_test, vocabulary, config_data)
...
def get_coco_dataloader(img_ids_file_path, imgs_root_dir, annotation_file_path, 
    ...
    ann_ids = [coco_obj.imgToAnns[img_ids[i]][j]['id'] \
                for i in range(0, len(img_ids)) for j in
    ...
    dataset = CocoDataset(root=imgs_root_dir,
                          ...
                          rotation=config_data['dataset']['rotation'],
                          hflip=config_data['dataset']['hflip'],
                          vflip=config_data['dataset']['vflip'] )
